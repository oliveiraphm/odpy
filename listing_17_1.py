...
#sns.lineplot(data=total_days_df, ax=ax[0])
#total_days_df['Diff'] = total_days_df['Purchases'] - total_days_df['Sales']
#sns.lineplot(data=total_days_df['Diff'], ax=ax[1])
#plt.tight_layout()
#plt.show()
...
jan_sales_df.loc['2023-01-15'] = 4033
...
#ax.axhline(daily_median)
#ax.axhline(upper_limit)
#ax.fill_between(jan_sales_df.index, lower_limit, upper_limit, alpha=0.1)
#sns.lineplot(data=jan_sales_df)
#sns.scatterplot(x=[x[0] for x in flagged_vals], y=[x[1] for x in flagged_vals], marker='*', s=500)
#plt.show()
...
for v in range(2, len(values)-2):
    smooth_values.append((values[v-2] + values[v-1] + values[v] + values[v+1] + values[v+2])/5)
